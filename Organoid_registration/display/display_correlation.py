...
MRI_METRICS = {
    "FA": {
        "path": os.path.join(WORK_DIR, "registered_Addition_native_Bvalue_finaltest/registered_masked/dti_FA_final_LAS_registered_native_masked.nii.gz"),
        "ylabel": "Mean registered FA",
    },
    "MD": {
        "path": os.path.join(WORK_DIR, "registered_Addition_native_Bvalue_finaltest/registered_masked/dti_MD_final_LAS_registered_native_masked.nii.gz"),
        "ylabel": "Mean registered MD",
    },
    "S0": {
        "path": os.path.join(WORK_DIR, "registered_Addition_native_Bvalue_finaltest/registered_masked/dti_S0_final_LAS_registered_native_masked.nii.gz"),
...
if ENABLE_DMRI_RESAMPLING:
    ...
    OUTPUT_DIR = os.path.join(WORK_DIR, f"correlation_results_finaltest_resampled_{SPACING_TAG}um")
else:
    OUTPUT_DIR = os.path.join(WORK_DIR, "correlation_results_finaltest_native_voxel1")
...
for metric_name, info in analysis_metric_data.items():
    ...
    print(
        f"[INFO] {metric_name} non-zero analysis voxels before filtering:",
        int(valid.sum()),
    )
